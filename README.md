
<h1 align="center">🧩 WFORK-Fronend-Page</h1>

<p align="center">
  Основная страница сообщества
</p>

<p align="center">
  <!-- GitHub badges -->
  <a href="https://github.com/wfork-org/WFORK-Fronend-Page/stargazers">
    <img alt="GitHub stars" src="https://img.shields.io/github/stars/wfork-org/WFORK-Fronend-Page?style=for-the-badge&logo=github" />
  </a>
  <a href="https://github.com/wfork-org/WFORK-Fronend-Page/forks">
    <img alt="GitHub forks" src="https://img.shields.io/github/forks/wfork-org/WFORK-Fronend-Page?style=for-the-badge&logo=github" />
  </a>
  <img alt="Last commit" src="https://img.shields.io/github/last-commit/wfork-org/WFORK-Fronend-Page?style=for-the-badge" />
</p>

<br/>
## 🚀 Live Website

> **Try it instantly:**  
> https://translateme.online/

<br/>

## 📸 Preview
<p align="center">
  <img src="screenshot.png" alt="WFORK-Fronend-Page">
</p>

<br/>


## ⚙️ RUN

docker compose up -d --build 

## 🤝 Contributing

Fork → branch → commit feat/fix

<details open>
  <summary>DEV Log</summary>


- [ ] v0.0.9 - создать нижнию панель с ссылкой на телеграм группу 
- [ ] v0.0.8 - исправить видео ролик, надо захостить прямой доступ к видео 
- [x] v0.0.7 - добавлен screenshot в README.md
- [x] v0.0.6 - исправлены ссылки в шамке md
- [x] v0.0.5 - добавлен логотип 
- [x] v0.0.4 - добавлена кнопка User Timer
- [x] v0.0.3 - добавлена кнопка wiki
- [x] v0.0.2 - добавлена кнопка регистрации
- [x] v0.0.1 - добавлена кнопка логин


<details>
  <summary>Github CHEATSHEET</summary>

## Update repository

git add .  
git commit -m "v0.0.7 - добавлен screenshot в README.md"  
git push


</details>



## 📄 License

Licensed under the MIT License.

<p align="center"><sub>Made with ❤️ &nbsp;</sub></p>
